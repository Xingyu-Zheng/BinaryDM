...
from ldm.modules.diffusionmodules.util_binarydm import make_beta_schedule, extract_into_tensor, noise_like
from ldm.modules.diffusionmodules.openaimodel_binarydm import TimestepEmbedSequential, BNNConv2d, BNNConv1d, BNNLinear
...
class LatentDiffusion(DDPM):
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        global fp_model
        if fp_model is None:
            fp_model = self.model2
            for module in fp_model.model.diffusion_model.bnn_modules:
                module.bnn_mode = 'full'

        for name, module in fp_model.model.diffusion_model.named_modules():
            if isinstance(module, TimestepEmbedSequential):
                hooks.append(module.register_forward_hook(t_forward_hook))
        with torch.no_grad():
            fp_model.apply_model(x_noisy, t, cond).detach()
        for hook in hooks:
            hook.remove()
        ### student
        for name, module in self.model.diffusion_model.named_modules():
            if isinstance(module, TimestepEmbedSequential):
                hooks.append(module.register_forward_hook(s_forward_hook))
        ...
        for (s_feature, t_feature) in zip(s_features, t_features):
            try:
                K = self.pca_V_list[index].detach()
            except:
                K = torch.pca_lowrank(t_feature.reshape(-1,t_feature.shape[-1]), q=t_feature.shape[-1]//4 if t_feature.shape[-1]//4 > 2 else 2, center=True, niter=2)[2].detach()
                self.pca_V_list.append(K)
            try:
                if loss_distil is None:
                    loss_distil = nn.MSELoss()(s_feature@K, t_feature@K)
                else:
                    loss_distil = loss_distil + nn.MSELoss()(s_feature@K, t_feature@K)
            except:
